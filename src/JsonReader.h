...
//
#ifndef JFSON_JSONREADER_H
#define JFSON_JSONREADER_H
...
#define CALL(expr) \
    ...
    {
        // unicode stuff from Milo Yip's tutorial
        unsigned u = 0;
        for (int i = 0; i < 4; i++) {
            u <<= 4;
    ...
    static void parseNumber(ReadStream& is, Handler& handler)
    {
        ...
        if (is.peek() == 'N') {
            parseLiteral(is, handler, "NaN", JSON_DOUBLE);
            return;
        }
        else if (is.peek() == 'I') {
            parseLiteral(is, handler, "Infinity", JSON_DOUBLE);
            return;
        }
        ...
        auto expectType = JSON_NULL;

        if (is.peek() == '.') {
            expectType = JSON_DOUBLE;
            is.next();
            if (!isDigit(is.peek()))
                throw Exception(PARSE_BAD_VALUE);
            is.next();
        ...
        if (is.peek() == 'i') {
            ...
            switch (is.next())
            {
                case '3':
                    if (is.next() != '2')
                        throw Exception(PARSE_BAD_VALUE);
                    expectType = JSON_INT32;
                    break;
                case '6':
                    if (is.next() != '4')
                        throw Exception(PARSE_BAD_VALUE);
                    expectType = JSON_INT64;
                    break;
                default:
                    throw Exception(PARSE_BAD_VALUE);
            }
        ...
        try {
            ...
            if (expectType == JSON_DOUBLE) {
                ...
                if (expectType == JSON_INT64)
                {
                    CALL(handler.Int64(i64));
                }
                else if (expectType == JSON_INT32)
                {
...
#undef CALL
    ...
    static void parseValue(ReadStream& is, Handler& handler)
    {
        ...
        switch (is.peek()) {
            case 'n': return parseLiteral(is, handler, "null", JSON_NULL);
            case 't': return parseLiteral(is, handler, "true", JSON_BOOL);
            case 'f': return parseLiteral(is, handler, "false", JSON_BOOL);
